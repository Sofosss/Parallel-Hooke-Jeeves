...
fn work(
    optimizer: HJOptimizer<impl Fn(&Vec<f64>) -> f64>, init_mech: &mut InitMech, rank: usize, chunk: usize,
...
fn main() -> ExitCode {
    ...
    if rank == 0 {
        println!(">> MPI Hooke-Jeeves Global Search Optimization <<");
    }
    ...
    let mut init_mech = if let Some(path) = file_path {
        ...
        println!(
            "INFO [Process {}]: Initializing the starting point using a uniform distribution generator with seed: {}",
            rank, gen_seed
        );
        InitMech::Random(rand::rngs::StdRng::seed_from_u64(gen_seed))
    };
    let (local_min, end_pt, trial, iterations, funevals) = work(optimizer, &mut init_mech, rank, chunk);

    let mut total_funevals: usize = 0;
    let root_process = world.process_at_rank(0);

    if rank == 0 {
        root_process.reduce_into_root(&funevals, &mut total_funevals, mpi::collective::SystemOperation::sum());
    } else {
        root_process.reduce_into(&funevals, mpi::collective::SystemOperation::sum());
    }

    let mut best_fx = local_min;

    if rank == 0 {
        let mut best_fx_collection = vec![local_min; size];
        root_process.gather_into_root(&local_min, &mut best_fx_collection);

        best_fx = best_fx_collection.iter().fold(f64::MAX, |acc, &el| if el < acc { el } else { acc });
    } else {
        root_process.gather_into(&local_min);
    }
    ...
    if local_min == best_fx {
        ...
        for (i, pt) in end_pt.iter().enumerate() {
            println!("x[{i:3}] = {:15.7e}", pt);
        }
        println!("f(x) = {:15.7e}", local_min);
    }
